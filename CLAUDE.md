...
**Tool lockdown.** Every agent declares an explicit `tools:` list in its frontmatter, and the **designer** and the **orchestrator** are the only ones that keep `Agent` — the designer so it can spawn its own `explorer` subagents when the exploration it was handed leaves questions unanswered, the orchestrator because spawning is its whole job. Everyone else reads code directly instead of delegating: reviewers, the judge, the workflow-scanner, the explorer, the refiner and the eli5-explainer all get `Read, Write, Edit, Bash, Grep, Glob` — `Edit` so an agent can revise its own artifact without rewriting it from scratch — and the implementer and comment-rewriter keep the lists they already had.
...
- **Spawns are anonymous.** No agent passes a `name` (or any human-readable agent-name field) on an `Agent` call — `subagent_type` and `prompt` are the whole call. Naming subagents flips the harness into a chatty "team" presentation, and these are one-shots, not colleagues. The rule is written into the orchestrator and the designer, the only two agents that still have the `Agent` tool.
