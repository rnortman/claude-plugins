...
- **prepass-reviewer** — thin two-lane pre-pass, pinned to Sonnet. Slop lane (diff only): non-comment LLM writing tells, obvious unhandled cases, workarounds visible on the face of the diff (comment quality is the comment-rewriter's job, handled by direct edit before the pre-pass runs). Scope lane (diff + design doc (+ deltas) + implementation log): did this round deliver its log-claimed slice (and, on the final `done` round, is the whole design accounted for), does every log claim trace to the effective design or a delta (else it's undesigned drift), are punts explicit and justified. Round-aware — the orchestrator tells it whether the round is intermediate or final. Can ESCALATE directly on aggregate missing scope.
