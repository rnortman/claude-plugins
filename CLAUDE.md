...
**Workflow artifacts are an audit trail — never overwritten.** The review artifacts (`notes-*`, `dispositions-*`, `judge-verdict-*`, `escalation-*`) are not ground truth for the current state of the code — the code is that — but they are 100% ground truth for what the workflow did and what happened at each step. Because they are an audit trail, they are never clobbered: every review round and every rework attempt writes its own durably-numbered file, keyed by round `r<R>` (review pass for requirements/design, implementation round for pre-pass/deep) and rework attempt `a<A>` (1 = initial, 2 = the one rework round) — e.g. `judge-verdict-deep-r2-a1.md`. The only in-place growth permitted is the append-only `implementation-log.md`; the four spec docs are edited in place only while being drafted pre-freeze, then frozen. The orchestrator never writes to a path that already exists.
