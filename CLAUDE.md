...
- **prepass-reviewer** — thin two-lane pre-pass, pinned to Sonnet. Slop lane (diff only): non-comment LLM writing tells, obvious unhandled cases, workarounds visible on the face of the diff (comment quality is the comment-reviewer's lane). Scope lane (diff + design doc (+ deltas) + implementation log): did this round deliver its log-claimed slice (and, on the final `done` round, is the whole design accounted for), does every log claim trace to the effective design or a delta (else it's undesigned drift), are punts explicit and justified. Round-aware — the orchestrator tells it whether the round is intermediate or final. Can ESCALATE directly on aggregate missing scope.
- **comment-reviewer** — pre-pass, runs in parallel with the prepass-reviewer, pinned to Sonnet. Enforces the comment standard over the round diff's added/modified comments — nine near-mechanical rules (no ephemeral-doc references, no narration, no remote-internals descriptions, contract doc comments on public items, invariant comments encouraged, self-contained why-comments, how-comments only for the irreducible, no commented-out code / slugless TODOs, generic identities in examples). Default disposition: delete. Pre-existing bad comments never authorize new ones.
