...
The `/simplify` skill is a user-facing convenience for ad-hoc review outside the full workflow — it runs the citizen-reviewer (quality + reuse + efficiency) and applies the fixes. The `/cleanup-editor` skill is invoked by the designer (and is available to any author) to self-clean a draft. The `/configure-models` skill generates and edits the local model-override config consumed by the `model-override` hook.
