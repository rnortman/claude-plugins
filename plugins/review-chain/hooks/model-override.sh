#!/usr/bin/env bash
#
# review-chain PreToolUse hook — override the model for agent spawns from local
# config and/or env vars, without editing agent files or publishing a new
# plugin version.
#
# Fires on every Agent/Task spawn and resolves a model for the spawned
# subagent_type by this precedence (first hit wins):
#
...
#   (none) -> pass through unchanged; the agent's frontmatter pin governs.
#
...
# generated config template (scripts/gen-model-config.sh).
#
...
# "no override".
#
# The built-in Explore agent is left to intercept-explore.sh (which also has a
# deny mode); this hook ignores subagent_type == "Explore".
#
...
# 3. explicit override already on the spawn: if present and no env override,
#    respect the orchestrator's deliberate choice and pass through.
if [ -z "$MODEL" ]; then
...
case "$MODEL" in
  ""|inherit|default|-) exit 0 ;;
esac

# Inject the resolved model into the tool input.
UPDATED=$(printf '%s' "$INPUT" | jq -c --arg m "$MODEL" '.tool_input + {model: $m}') || exit 0
jq -nc --argjson ui "$UPDATED" --arg m "$MODEL" --arg a "$SUBAGENT" '{
  hookSpecificOutput: {
    hookEventName: "PreToolUse",
    permissionDecision: "allow",
    permissionDecisionReason: ("review-chain: model override -> " + $m + " for " + $a),
    updatedInput: $ui
  }
}'
exit 0
