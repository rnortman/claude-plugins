...
- An **outcome token** where the workflow branches on it — `done` | `in progress`, `committed` | `split` (+ stash ref), `swept` | `no changes`, `HANDOFF`, `HOOK-FAILURE`, `CLARIFICATION-NEEDED`, `APPROVED` | `REWORK` | `ESCALATE`, `DELTA` | `RESUME`, `FINDINGS` | `NOTHING-TO-REPORT` | `CONTINUE`.
...
Only after the user approves: freeze the delta (commit / checksum), then loop to step 21 — the round continues with increments that implement it, passing the new delta path along with the design and prior deltas. This gate lands mid-implementation and pauses it. That is intended — implementation waits on a human whenever the spec moves.
...
- `DELTA` + delta path + dispositions path → **delta review** from D2, then its user gate, then step 21: the round continues with increments that implement the delta. When the round next closes, its review runs in full over `round base..HEAD` under a bumped `R` — `R` numbers review passes; the round base is what makes it the same round.
...
42. User asks for changes instead of approving → capture the request (user-supplied notes doc → use that path; chat directives → write verbatim to `notes-shipgate-user-<K>.md`, `K` = 1, 2, … — never reuse a prior file), then route it like any other change: a design change goes through **delta review** in full from D1, gate included; a code change is a new implementation round. Open a new round for it — round base = current HEAD, increment counter 0, bump `R` — and loop to step 21 with the user-notes path (and any new delta path). Normal increments, normal caps, normal pre-pass + deep + round-close scan over the new commits only; `done` brings it back to step 39.
