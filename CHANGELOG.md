...
A round that has been reviewed is finished. The orchestrator was treating a design delta written *after* a round's full review and round-close scan as one more increment of that round — and then closing the round a second time and re-running the entire review chain over it. The 0.34.0 triage rule ("the round continues with increments that implement the delta; its review re-runs in full under a bumped `R`") was written for a delta that arrives while increments are still landing, and said nothing about one that arrives during or after review, so the orchestrator applied it everywhere.

- **orchestrator:** rounds now carry an explicit **round state** — *open* until the pre-pass reviewer is spawned, *in review* from that spawn until the round-close scan replies, then *finished*. An open round takes increments; a round in review or finished never does, and nothing reopens one. New **where a delta lands** section, the single rule for implementing a gated delta, keyed on the round state at the stop that produced it: **open** (implementer `CLARIFICATION-NEEDED`) → the round continues with increments; **mid-review** (pre-pass reviewer, respond, or judge `ESCALATE`) → no increment — a fresh respond-mode implementer at the stopped step lands as much of the delta as the pass needs to finish (pre-pass stop → responder step 26; respond stop → that respond step re-spawned; judge stop → the rework slot, with `A` advanced where a path would collide), and its reply carries a required **delta completion token**: `delta-complete`, or `delta-remaining` (missing = remaining), which in a final round **revokes `done`** — the round still finishes its review, but its round-close scan is told `intermediate`, it takes the silent squash instead of the ship-gate, and the next round implements the remainder; **finished** (gate-scanner `ESCALATE`, or a user change at or after the ship-gate) → the round is done, intermediate rounds squash, a final round skips its ship-gate, and a fresh round at the current HEAD implements the delta. The triage `DELTA` route and the delta-gate close both point at that rule; the "review re-runs in full under a bumped `R`" clause is gone. Ship-gate step 42 says the same thing for user-directed spec changes. New Principles bullet and Never entry: never spawn an increment into a round whose pre-pass reviewer has been spawned, never run any part of a round's review twice.
- **implementer:** new **Landing a delta** section in respond mode. A delta that arrives with the triage dispositions is the responder's to land — the minimum the review needs to finish, its own call on where that line is — and the reply then carries `delta-complete` or `delta-remaining`. That token is the only channel by which an earlier `done` gets revoked, because the log and the dispositions doc never reach the orchestrator; the remainder is not written down as a "remaining" section but recovered by the next implementer from the effective design against the log, like any other unimplemented scope.

### 0.34.0 — 2026-09-04

The front half of the workflow is gone: no pre-exploration, no requirements phase. The orchestrator captures the user's request **verbatim** into `user-request.md` (or uses one the user points at) and spawns the designer directly. The designer explores the codebase itself, under rules that came from watching explorers fail: broad explorations miss details, get them wrong, and hallucinate them, because their attention is too diffuse. And the designer is now the first responder to every mid-implementation stop — the workflow used to hand escalations to the user cold, with a scanner report to explain them; now a fresh designer rules on them, and the user is only pulled in when something genuinely needs their judgment.

- **Deleted:** `requirements-refiner`, `requirements-reviewer`. `requirements.md`, `requirements-delta-<N>.md`, the requirements gate, and the requirements cast of the delta chain are all gone. `user-request.md` is the request; it is frozen with the design and never edited by anyone.
- **orchestrator:** setup captures the request as a scribe (verbatim, or an existing file); no explorer spawn — the designer runs its own; step numbering compressed accordingly. New **triage** section: every post-freeze `ESCALATE` (pre-pass reviewer, implementer respond, judge, gate scanner) and every `CLARIFICATION-NEEDED` spawns a fresh `designer` in triage mode, and its reply routes — `RESUME` continues past the stop as if it had passed (dispositions path handed to the next spawn, listed at the next gate), `DELTA` enters delta review at the reviewer step and then loops back into the same round (whose review re-runs in full under a bumped `R` when it next closes), `ESCALATE` stops for the user. `HOOK-FAILURE`, toolchain stops, and modified frozen docs still go straight to the user; design-review and delta-review escalations surface at the gate they were headed to. Ship-gate step 40 spawns the new `changelog-author` after the squash and offers *its* HEAD for push. `counter = 5` became `counter ≥ 5` so a post-delta increment in a capped round still closes it.
- **designer:** new **Exploring** section — at most three *related* questions per explorer (three disjoint questions are three explorers); a broad orienting survey may come first but nothing in it is relied on until a narrow follow-up ground-truths it; explorations run **strictly serially**, and the designer spawns one, ends its turn, and waits for the notification — no watchers, no parallel work; each lands at `exploration-<N>.md`. Almost every draft starts with one. New **Other agents are not authorities** section governing respond and triage: decide from the forest whether the problem is real, whether the fix helps or hurts at the big-picture level, whether it's worth it, and what the right fix is independent of what was proposed; zero weight to the fact that code already exists; ignore complaints about the workflow itself. New **triage** mode: always writes `dispositions-triage-<K>.md` (per item: **No-Change** with the corrected reading, or **Delta**); writes a `design-delta-<N>.md` only if something must actually change — accepting the status quo or correcting the record needs no delta. The "prefer serial" paragraph (a mild preference) is replaced by the rule.
- **explorer:** scoped to the few related questions it is asked; report organized by question; says plainly what it could not find. The "cast wide" line and the append-to-existing-report line are gone.
- **design-reviewer:** reviews against the user's request and the code — no refined request, no exploration report to lean on. Delta check 1 now also covers "the code deviates from the design", since triage deltas can be about code, and the change input is the triage dispositions plus the doc that triggered them.
- **workflow-scanner:** **explain** mode removed — designer triage replaces it; a gate `ESCALATE` report is the triage input, and the reply vocabulary loses `EXPLAINED`. Inputs name the user request instead of the refined request.
- **judge, eli5-explainer, implementer, comment-rewriter:** requirements/exploration inputs removed. The judge and the implementer accept a triage dispositions path and treat it as authoritative on the items it rules on; the implementer's clarification doc is now written for a designer who will triage it rather than for a delta reviewer.
- **New agent `changelog-author`**, pinned to `claude-opus-4-6[1M]`: after the ship squash, reads the ADR docs and the squash diff, writes (or edits the implementers' own) changelog entry in the unreleased section — highlights for humans, most important first, jargon moderated, small fixes as brief later bullets — and amends it into the squash with a rewritten commit message that is precise and detailed because its readers are often LLMs. No changelog in the project → it only rewrites the message.
- **CLAUDE.md** gains a **Prompt economy** rule: every token in an agent definition is paid on every spawn, so state each rule once where it belongs.
...
- **New agent: `workflow-scanner`** (`claude-opus-5[1M]`), the only agent whose report is written for the **user** — plain ELI5-level prose carrying its own context, because the reader has read none of the artifacts. Mode **scan** hunts five things: problems filed instead of escalated, drift from the effective design, skipped workflow steps (a wave that never ran, a verdict with no dispositions behind it, a reused artifact filename, unreviewed commits), agents accepting by default where they were supposed to be adversarial (findings that vanish between notes and dispositions, "Fixed" with no file:line, Won't-Do that is really "out of scope", rubric-failing TODOs, a judge that didn't engage the dispute), and anything else that looks wrong. Mode **explain** covers escalation stops — what was being built, what the agent hit, why it stopped, what each option costs. Not a reviewer: no responder, no judge, no chain, no verdict. It audits the process and is explicitly barred from re-reviewing the code or second-guessing design decisions on their merits.
- **Nothing found means the report is literally "Nothing to report."** Padding with a recap of the rounds is a failure mode, not thoroughness — mundane summary trains the reader to skim, so the one time there *is* something it gets skimmed too.
- **The user never arbitrates cold.** Every stop that surfaces an escalation or `CLARIFICATION-NEEDED` — responder or reviewer `ESCALATE`, judge `ESCALATE`, `HOOK-FAILURE`, a halt on a modified frozen doc — now also spawns the scanner in explain mode and surfaces both paths. Also spawnable on user request ("what's actually going on", "is anything getting swept under the rug"). Reports land at `workflow-scan-<K>.md` in the audit trail; new outcome tokens `FINDINGS` / `NOTHING-TO-REPORT` / `EXPLAINED`.
- **The no-thumb-on-the-scale rule extends to the scanner.** The orchestrator passes the round/increment range and nothing else, may not spawn it because *it* suspects something is off, and may not read the report or pass it to any subagent. The scanner enumerates the artifact directory itself rather than being handed a file list — which is how it can notice a file that *should* exist and doesn't.
- **implementer: new `Writing it down is not surfacing it` section**, placed before the mode-specific instructions. Names who actually reads each artifact, lists the tells that mean escalate instead ("this will break Z, but that's out of scope", "I couldn't do this honestly so I did the nearest thing"), and states that the log entry is *in addition to* the escalation, never instead of it. Fenced against the inverse — ordinary deviations, rubric-passing TODOs, and normal scope splits stay log material; the test is whether an owner would want to be interrupted.
- **Spawns are anonymous.** No agent passes a `name` (or any human-readable agent-name field) on an `Agent` call — `subagent_type` and `prompt` are the whole call. A name flips the harness into a chatty "team" presentation that clutters the transcript and invites treating one-shots as persistent colleagues. Written into the orchestrator (plus a **Never** bullet and a `// no name` comment in the spawn template), the designer, and the requirements-refiner — the three agents that spawn.

### 0.27.0 — 2026-07-27

Post-freeze spec deltas get reviewed. An implementer replying `CLARIFICATION-NEEDED` used to get a design delta written from its own complaint and handed straight back to it — no reviewer, no judge, no user gate. That was the workflow's one unreviewed path into the spec, and the worst place to have one: the pre-pass scope lane accepts a delta as authority, so a delta is precisely the document that converts undesigned drift into designed work. Skipping its review didn't just omit a check, it disarmed one.

- **orchestrator: new `delta review` section** (steps D1–D6, lettered so the numbered workflow didn't renumber). Every post-freeze delta runs the chain the original spec ran: `designer` mode delta → `design-reviewer` → designer respond → `judge` (one rework round) → `eli5-explainer` (`design-eli5-delta-<N>.md`) → **user gate**. Requirements deltas run the same chain with the refiner/`requirements-reviewer` cast and no eli5 step. Only after the gate may a delta reach an implementer.
- **Delta artifacts reuse the existing phase names on a design-review `R` that continues across the freeze** instead of resetting — design review ending at `R`=2 makes delta 1's review `R`=3. No new filename vocabulary and no collisions with the design phase's own files. Spelled out that this is the design-phase `R`, not the implementation-round `R`, since both are called `R`.
- **orchestrator step 24 rewritten.** It now names the `CLARIFICATION-NEEDED` token (previously it only said "clarification-needed doc", and the token list read as refiner-only despite the implementer emitting it too), routes through D1–D6 *and* the gate, resumes at step 21 as a normal next increment, and notes the round pauses with the counter and round base unmoved. New **Never** bullets: never spawn an implementer against an un-reviewed or un-gated delta, and never decide on your own that the spec needs changing — deltas come from `CLARIFICATION-NEEDED` or the user.
- **A delta joins the frozen set on gate approval, not when written.** The old rule froze it the instant the designer finished, which would have made it unfixable by its own review responder. Until its gate clears, a delta is a draft; the frozen-set definition and the freeze check follow suit.
- **The ship-gate's mid-flow user revision is the one gate exemption** — it runs D1–D6, but re-entry to the ship-gate is already the human gate, so no second one.
- **Clarification docs get numbered, orchestrator-assigned paths** (`clarification-needed-r<R>-i<n>.md`), passed on the spawn like escalation targets. The old fixed `clarification-needed.md` overwrote itself on a second occurrence, and the doc is now a reviewed input cited by a reviewer and a judge — audit trail, not scratch.
- **design-reviewer and requirements-reviewer: delta sections.** Four checks for a design delta, in order — is the stated problem real (verify the implementer's claim against source before anything else; a delta built on a misreading is the whole finding), is the delta the *minimal* fix (an agent that has to do the work now has license to rewrite the spec, so scope creep here is nearly free and carries a delta's authority), does the composite still cover the requirements, and is it conflict-free with everything it supersedes named. Three analogous checks for a requirements delta.
- **designer and requirements-refiner: respond mode knows delta context** — fixes go in the delta doc, and the delta may supersede more of the frozen doc than it originally did, but only by reference, never by editing frozen text. **eli5-explainer** gains a delta rendering mode that explains the change against the original rather than re-explaining the whole design. **judge** inputs document the delta-plus-original shape and that ground truth is the composite read in order.
- **implementer: the clarification doc is written for its audience.** It writes to the given target path, and is told what happens next — a reviewer fact-checks the claimed problem against source, a judge adjudicates, the user approves or rejects, and implementation is stopped the whole time. An alternative reaching further than the problem requires comes back rejected.

### 0.26.0 — 2026-07-27

The orchestrator stops putting its thumb on the scale. It has read no code, no design, and no log — by design — so any hint it offers a subagent is a bet placed with information it doesn't have. Summaries are gone from every reply so there is nothing to improvise from, and implementer scope policing switches to added lines only, with no time limits at all.

- **orchestrator: no-hints rule, stated with its reason.** New identity framing up front — not a manager, tech lead, or reviewer of reviewers, but the office coordinator that starts jobs in order and hands over paths. `## Prompts` now leads with "Never put your thumb on the scale. Especially not with reviewers," explains *why* (least-informed participant, words carrying unearned authority, a hinted reviewer confirms the hint and misses the real bug), and lists what may never be added: focus areas, risk assessments, restated rubrics, prior-round recaps, reassurance/de-scoping, expected findings or verdicts. "Being helpful is the failure mode… that pull *is* the bug." Reinforced in Principles and as three new **Never** bullets.
- **orchestrator: subagent replies carry no summaries.** New `## Replies` section defines the contract — paths, commit hashes, and an outcome token where the workflow branches (`done`/`in progress`, `committed`/`split`, `swept`/`no changes`, `HANDOFF`, `HOOK-FAILURE`, `READY-FOR-REVIEW`/`CLARIFICATION-NEEDED`, `APPROVED`/`REWORK`/`ESCALATE`). A summary is something the orchestrator would be tempted to act on, so it isn't given one. Prose that arrives anyway is ignored, never forwarded, never allowed to color the next prompt; asking an agent "what did you find?" is forbidden.
- **All 15 agents: reply specs rewritten** from `≤3 lines + path` to paths plus outcome token. The judge drops its findings-count breakdown; the comment-rewriter drops its scale summary but keeps its out-of-scope observation channel by writing `comment-sweep-observations-<HEAD>.md` (it deletes the comment, so no reviewer would otherwise see a comment admitting a bug) — the orchestrator surfaces that path at the next gate without reading it.
- **orchestrator: the verbatim recency line on incremental implementer spawns is removed.** It was tool-call ordering, not scope guidance, but it was the sole precedent for the orchestrator appending behavioral text to a prompt. The instruction now lives as the last line of `implementer.md`, keeping the recency effect without the orchestrator holding the pen. Step 21 now says explicitly to pass nothing about scope.
- **Implementer scope is policed in added lines, never elapsed time.** The watchdog's `or 30 minutes` / `or 45 minutes` arms are gone; thresholds are 900 added lines (warn) and 1,200 (terminate), and the `SendMessage` templates no longer quote elapsed minutes. Timers are demoted to alarm clocks — "the clock is your alarm, never your evidence." A 90-minute implementer sitting at 400 added lines is doing exactly what's wanted.
- **The metric is insertions only.** Deletions never count and never offset, so a 900-insertion/900-deletion refactor is a 900-line increment and no one buys headroom by deleting code. Measurement moves from `git diff --stat` to `git diff --numstat | awk '{s+=$1}'`. Propagated to the implementer's own sizing rubric (500–700 *added* lines).
- **Rounds close on lines or increment count, whichever comes first.** New ~4,000-added-line round budget; five increments is a ceiling, not a target, so three 1,100-line increments close a round as properly as five 400-line ones. Step 22 measures the round total after each increment and step 23 gained an early-close branch at ≥~3,500, framed as judgment rather than arithmetic. "5-cap" phrasing throughout becomes "line- or increment-capped."
